...
class WPML_Displayed_String_Filter extends WPML_WPDB_And_SP_User {
	...
	protected function string_from_registered( $name, $context = "", $untranslated_text = "" ) {
		if ( $this->cache_is_warm === false ) {
			...
			if ( $this->language !== $this->sitepress->get_current_language() ) {
				$this->warm_cache( $name, $context, $untranslated_text );
			} else {
				$this->warm_cache();
			}
	...
	protected function warm_cache( $name = null, $context = null, $untranslated_value = "" ) {
		...
		$res_query   = "
					SELECT
						IF(st.value IS NULL, s.value, st.value) AS tra,
						s.value AS org,
						s.domain_name_context_md5 AS ctx,
					 	IF(st.value IS NULL, '1', '0') AS missing
		...
		foreach ( $res as $str ) {
			...
			$this->untranslated_cache[ $str['ctx'] ] = $str['missing'] === "1"  ? true : null;
